#!/usr/bin/env python3
"""
Build a compact cross-year rider index for the Giro d'Italia Riders page.

Reads from cycling-app/src/data/giro/gc_by_stage_*.json and writes
cycling-app/src/data/giro/riders_index.json in the same format as the
TDF riders_index.json.
...
GIRO_DATA_DIR = os.path.join(HERE, "..", "cycling-app", "src", "data", "giro")
OUT_PATH = os.path.join(GIRO_DATA_DIR, "riders_index.json")


def main():
    riders = {}
    team_names = set()
    files = sorted(glob.glob(os.path.join(GIRO_DATA_DIR, "gc_by_stage_*.json")))
    if not files:
        print("No Giro gc_by_stage_*.json files found")
        return
