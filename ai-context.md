...
- **The stage-1 GC fallback is FIXED IN CODE; the archive is NOT re-ingested, and that is Eric's call.** `ingest_race` now runs the approximation only where the stage has **no other source** — no rider with a published `gc_pos` and no `gc_standings` entry — so it can never again land on top of a real ladder. 4 mutations, all caught.
  **Why the archive was left alone.** "Prefer `gc_standings`" turned out to be empty: for Vuelta 1996 st1 that file holds 103 riders and **100 of them have a rank of `None`** — PCS genuinely publishes only three GC positions after that stage. So not fabricating means NULL, not a better number. Re-ingested into a copy, stage 1 goes from 180 GC rows to **3**, and the 1996 export drops from 182 riders with a stage-1 GC to 3. Vuelta 1980 goes 53 -> 1, 1983 100 -> 1, while 1971 loses nothing. **A file-level estimate said only 5,816 rows would be lost and was wrong** — the re-ingests are the number to trust, and they say most of stage 1 disappears across roughly 200 editions.
  So the trade is: **the values are ours, not PCS's, and removing them is truthful — but it empties the stage-1 GC column for most of the archive.** Deriving the whole ladder from the finishing order instead is worse, not better: it would contradict the three positions PCS DOES publish (Galdeano is 3rd on GC and 170th on the stage). Duplicated ranks go 1 -> 0 on every edition tested, so the fix works; it is the coverage that needs a decision.
- **748 stage-1 GC positions are held by more than one rider, because the ingest invents one.** `ingest_race` has a fallback — `if not gc_pos and n == 1` — that gives a stage-1 rider his **stage placing** as a GC position and his stage gap as a GC gap. PCS publishes a GC position for very few riders after stage 1: **three of 180** on Vuelta 1996 st1 (the top three, reordered by bonifications), so the fallback invents one for the other 177 and they collide with the three real ones. **26,315 of 35,928** stage-1 GC rows carry `gc_rank == stage_rank`.
  **This is what the "1,097 zero fillers" turned out to be, and nothing was nulled.** The count was written up on 2026-09-19 as PCS's `+0:00` filler read as a real zero — the sibling of the ITT defect — and that was wrong: **1,093 of the 1,095 rows are on stage 1 and carry `gc_rank == stage_rank`**. The zero is usually TRUE (a bunch rider did finish on the winner's time) and the RANK beside it is the fabricated half, so nulling the gaps would have removed the true value and left the false one. An ingest guard and a `null_gc_gap_filler.py` were written, tested, and **both thrown away** once the stage-1 signature showed up. Caught by reading two affected ladders before writing, not by a test.
  Only **2 rows** are the defect originally described: `rider/andris-nauduzs` on Giro 2004 st11 and st12, whose `gc_lag` cell reads `+0:00` while his stage gap is 15:20.
  `validate_db.check_gc_gap_zero_filler()` still reports the 1,095 — the DETECTOR was right and only the story attached to it was wrong — and now says not to null them. **The repair is an ingest question** (stop fabricating a stage-1 GC position, or prefer `gc_standings` for it), it would move 26,315 rows, and it is Eric's call.
