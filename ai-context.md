...
- **Four GC rows in the whole archive disagree with their own source page**, and they are two clean pairs rather than noise: **Tour 1926 st17** has `leon-parmentier2` (6th, 6,975s) and `albert-dejonghe` (7th, 7,760s) reversed, and **Tour 1957 st24** has `gastone-nencini` (1,563s) and `nino-defilippis` (1,677s) reversed. (A fifth, Giro 1982 st3's `giuseppe-saronni`, dropped out once absolute-time pages were refused — it was never a real disagreement.) Read [[project_name_swap_repair]] before touching the pairs — a re-scrape UNDOES a repair. **3,795 stages and 5 disagreeing rows is the headline**: the archive's GC is faithful to its source.
