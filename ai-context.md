...
- **113 stages hold a GC ladder that runs backwards** (was 342 before the relegation marker was recorded), reported by `validate_db.check_gc_gap_monotonicity()` and triaged by **`audit_gc_ladders.py`** (offline, read-only). The GC *is* the ranking of aggregate time, so a later rank stored closer to the leader is arithmetically impossible — Tour 1919 st3 has rank 23 at 30,667s and rank 24 at 1s. Found 2026-09-19 when the 241 tie-groups turned out to live in only 49 stages while 293 more were corrupt without anything colliding.
  ...
  **RESOLVED 2026-09-19, and the re-scrape was never needed: most of these are PCS's own and our values are right.** PCS prints an asterisk on a rider whose PLACE the jury changed while his TIME stood — "Tim Merlier relegated from 2nd to 89th" on Giro 2024 st11, whose page carries a Penalties & Fines tab saying so. His stored time is then smaller than that of the riders now ranked above him, which is exactly this shape. The separation is total: across every stored GC page, **717 asterisked rows are out of order and 227,820 unasterisked rows are not** — 100% against 0.00%. Our scraper keeps the marker in the raw cell (`*0:040:04`, the visible text and PCS's hidden sort span concatenated) and the ingest dropped it, so the contradiction downstream was the LOST MARKER, not a wrong number.
  **FIXED 2026-09-19.** `stage_results.relegated` records it; `gc_source.py` reads the page; `backfill_relegations.py` flagged **718 rows across 233 stages** from files already on disk; `ingest_race.py` sets the same flag from the same helper, so a rebuild recreates it; and both GC checks exempt a marked row. **342 stages -> 113**, and **29 ONE ROW, 16 PAIR, 68 LADDER** remain. Verified the way that matters: re-ingesting Giro 1984 into a copy reproduced all 21 flags with **0 rows changed, 0 lost, 0 new**, and the same re-ingest with the ingest change reverted dropped all 21 — so the flag demonstrably comes from the new code and survives a rebuild. NO RANK AND NO GAP WAS WRITTEN.
...
- **896 NULL `gc_gap_seconds` can be filled from pages already on disk**, across 20 editions (Tour 1923 alone holds 300, Tour 2006 164, Tour 1903 71). A NULL-fill overwrites nothing, so it carries no revert risk — but it has not been applied. Measured across 3,795 stages whose page passed the alignment gate.
- **Five GC rows in the whole archive disagree with their own source page**, and they are three clean findings rather than noise: **Tour 1926 st17** has `leon-parmentier2` (6th, 6,975s) and `albert-dejonghe` (7th, 7,760s) reversed; **Tour 1957 st24** has `gastone-nencini` (1,563s) and `nino-defilippis` (1,677s) reversed; **Giro 1982 st3** stores 68s for `giuseppe-saronni` where the page says 0. Read [[project_name_swap_repair]] before touching the pairs — a re-scrape UNDOES a repair. **3,795 stages and 5 disagreeing rows is the headline**: the archive's GC is faithful to its source.
