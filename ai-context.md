...
└── pipeline/                         # Data pipeline — not deployed
    ...
    │                                 #   18 entries as of 2026-08-15, all of which CHANGE the computed value:
    │                                 #   13 slowestFinisherTimeSeconds corrections + 5 totalElevationM nulls.
...
What survives is worth reading, because it documents where the DB genuinely can't be trusted: 13 Giro + 3 Vuelta `slowestFinisherTimeSeconds` corrections where the computed winner+max-gap runs high, Vuelta 1978 supplying a value the DB can't produce at all, and 5 Giro `totalElevationM: null` pins suppressing absurd partial sums (**1992 sums to 70 m** for a 3,000 km race).
...
**Static landing pages must be listed in TWO places, and now aren't.** `generate-race-pages.mjs` writes `<race>/index.html` from `race-page-meta.mjs`'s `RACES`, and `vite.config.ts` needs each one in `rollupOptions.input` or it never reaches `build/`. `classics/` was generated but not listed, so the footer's `/classics/` link 404'd on the live site **while working perfectly in dev**, where Vite serves the file straight off disk. `vite.config.ts` now derives its input map from the same `RACES` object. `race-page-meta.mjs` is data-only for exactly this reason — importing the generator would run its file writes every time the Vite config loads.
