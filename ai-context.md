...
**Closed 2026-09-12:**
- **The disqualification sweep is complete** across all 113 Tour, 109 Giro and 81 Vuelta editions: **1,487 results, 69 riders**. `disqualified=1` survives a re-ingest, exports carry it, and the frontend renders it. The Vuelta count said 80 and the rider count 67 until 2026-09-19; both had gone stale, which is why `TestAiContextHeadlineCounts` now derives all four from the database.
...
- **111 stages hold a GC ladder that runs backwards** (was 342 before PCS's time marker was recorded), reported by `validate_db.check_gc_gap_monotonicity()` and triaged by **`audit_gc_ladders.py`** (offline, read-only). The GC *is* the ranking of aggregate time, so a later rank stored closer to the leader is arithmetically impossible — Tour 1919 st3 has rank 23 at 30,667s and rank 24 at 1s. Found 2026-09-19 when the 241 tie-groups turned out to live in only 49 stages while 293 more were corrupt without anything colliding.
  **The offline repair was tried and rejected, and the reason is the useful part.** *Rank is not recoverable from gap*: riders on equal aggregate time take DIFFERENT ranks, split by a tiebreak (sum of placings) we do not store — deriving rank from gap alone moves **71,806 rows, 18% of the archive**. Gap is not recoverable from rank for the same reason reversed. So the ladder proves a row is wrong without saying which value is right, and an offline "fix" would be fabrication.
  What the ladder CAN do is name the row and bound it, which is the triage: **29 ONE ROW** (removing exactly one of the step's two rows makes the ladder monotone, so that row is the suspect and its true gap is bounded by its neighbours — `rider/lucien-didier`, Tour 1978 st8, stores 4s where the ladder needs 289-304s), **15 PAIR** (either row would explain it; the evidence does not choose), **67 LADDER** (several steps, or neither removal is enough — re-fetch the classification whole). **24 stages hold TWO interleaved classifications**, one with `gc_rank` copied from `stage_rank`: Tour 1987 st1 stores ranks 13-18 twice, once at a flat 23s and once at the real prologue gaps. That is a ladder to REMOVE, not a value to correct.
  ...
  **FIXED 2026-09-19.** `stage_results.time_adjusted` records it; `gc_source.py` reads the page (with the alignment gate); `backfill_time_adjusted.py` flagged **868 rows across 260 stages** from files already on disk; `ingest_race.py` sets the same flag from the same helper, so a rebuild recreates it; and both GC checks exempt a marked row. **342 stages -> 111**, with **29 ONE ROW, 15 PAIR, 67 LADDER** left. The app shows PCS's own asterisk beside the rider in the GC legend — deliberately NOT the strikethrough a disqualification gets, because nothing was taken away from him — carried to the frontend as `adj: [stage numbers]` on 768 riders in 79 editions. Verified: re-ingesting Giro 1984 into a copy reproduced all 21 flags with **0 rows changed, 0 lost, 0 new**, and the same re-ingest with the ingest change reverted dropped all 21. **NO RANK AND NO GAP WAS WRITTEN**, and the 72 re-exported data files differ from their predecessors by the added key and nothing else.
  **Both tables are read** — the classification cell and the stage result cell, which are the same fact about the same row. Coverage is a FLOOR set by which scraper wrote each file: Roglič above is *not* flagged, because Vuelta 2022 has no `gc_pages` file and his row in `stage_16.json` carries a bare `+0:00`. His mark survives only in `classification_scrapes` HTML, which holds **1,022 more marked rows** we could claim and deliberately do not — a page there stacks the stage, GC, points, KOM and youth tables in `resTab` divs keyed by opaque numeric ids, and a mark in the points table says nothing about the GC time. Mis-scoping it would SUPPRESS real contradictions, which is the opposite of what the flag is for. Scope the tables first, or leave it.
  **Read the page by `source_slug`, never by stage number.** The 1978 Tour opens with a prologue, so its stage 8 is PCS's `stage-7`; reading `stage-8.json` puts a different rider on every row and nearly turned a faithful value into a reported defect.
...
- **1,097 GC gaps are stored as 0 where PCS published nothing**, across 30 stages — its `+0:00` filler read as a real zero instead of a NULL. Found 2026-09-19 and now reported by `validate_db.check_gc_gap_zero_filler()`. **The test has to be comparative**, which is the whole difficulty: **7,735** riders genuinely share the leader's time and nulling those would destroy ordinary bunch finishes. A zero is only impossible when a BETTER rank in the same stage already carries a positive gap. Worst: Vuelta 1996 (173), Tour 1971 (113), Tour 1980 (107). **28 of the 111 backwards GC ladders are really this one defect**, and 2 more stages are invisible to the ladder check — which is why it is reported as a cause rather than folded into that shape. **Same call as its sibling in `finish_time_seconds`, and still Eric's**: NULL is the honest value, but a re-ingest recreates every one from the same cells, so `ingest_race` has to stop reading them as zeros before the data can change.
- **718 NULL `gc_gap_seconds` can be filled from pages already on disk**, across 17 editions (Tour 1923 alone holds 300, Tour 1903 71, Tour 1919 49). `backfill_gc_gaps.py` prints the table and **fills NULLs only** — it refuses to overwrite a stored value, which is what makes it safe: nothing hand-researched can be destroyed by it. **NOT APPLIED**; it is a dry run until Eric has read the table.
...
- **One struck rider should be flagged and one should not**, researched 2026-09-19 after the sweep's re-verification found both.
  **`rider/tadej-valjavec`, Vuelta 2009 — READY TO APPLY, evidence is clean.** PCS strikes him at 39 in a classification on the 2009 GC page and the database flags none of his 21 rows. The Court of Arbitration for Sport set aside his exoneration on 2011-04-22, imposed a two-year ban and disqualified **all his results between 19 April and 30 September 2009** — the 2009 Vuelta (29 Aug – 20 Sep) sits inside that window, so the whole edition goes, not just the struck row. That is also how the archive already records the same thing: `juan-jose-cobo` has **all 21** of his Vuelta 2009 rows flagged. Not applied — a doping flag on a named rider is Eric's to confirm.
  **`rider/ramon-carretero`, Giro 2015 st1 — DO NOT APPLY.** The struck row reads `18 Carretero Ramón Southeast` with **no time and no gap**, and stage 1 of the 2015 Giro was a TEAM TIME TRIAL, where the page carries a team table beside the rider one. The database has him 197th on that stage and DNS on stage 2, so the struck 18 is not his stage placing and may not be about him individually at all. Wikipedia confirms he is suspended but gives neither the window nor the annulled results. **Scope the page's `resTab` tables before reading anything from it** — this is the same trap that makes the 1,022 extra marked rows in `classification_scrapes` unusable.
  Both were invisible to the sweep's own `--duplicated-rank1` entry point, because no rider was promoted into either vacated place — the shape that leaves no trace in our data.
