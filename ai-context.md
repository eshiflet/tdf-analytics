...
- **113 stages hold a GC ladder that runs backwards** (was 342 before the relegation marker was recorded), reported by `validate_db.check_gc_gap_monotonicity()` and triaged by **`audit_gc_ladders.py`** (offline, read-only). The GC *is* the ranking of aggregate time, so a later rank stored closer to the leader is arithmetically impossible — Tour 1919 st3 has rank 23 at 30,667s and rank 24 at 1s. Found 2026-09-19 when the 241 tie-groups turned out to live in only 49 stages while 293 more were corrupt without anything colliding.
  ...
  **FIXED 2026-09-19.** `stage_results.time_adjusted` records it; `gc_source.py` reads the page (with the alignment gate); `backfill_time_adjusted.py` flagged **718 rows across 233 stages** from files already on disk; `ingest_race.py` sets the same flag from the same helper, so a rebuild recreates it; and both GC checks exempt a marked row. **342 stages -> 113**, with **29 ONE ROW, 16 PAIR, 68 LADDER** left. The app shows PCS's own asterisk beside the rider in the GC legend — deliberately NOT the strikethrough a disqualification gets, because nothing was taken away from him — carried to the frontend as `adj: [stage numbers]` on 638 riders in 73 editions. Verified: re-ingesting Giro 1984 into a copy reproduced all 21 flags with **0 rows changed, 0 lost, 0 new**, and the same re-ingest with the ingest change reverted dropped all 21. **NO RANK AND NO GAP WAS WRITTEN**, and the 72 re-exported data files differ from their predecessors by the added key and nothing else.
  Still unread: the **stage result** cell carries the same marker on 139 rows, which is why Roglič above is not flagged in this column.
