...
- ~~**15 editions publish a "Slowest Finisher" that is really about 10th place.**~~ — **RESEARCHED 2026-09-19.** The All Races view charts `slowestFinisherTimeSeconds` as a red speed line, and for the 17 editions with no `gc_rank=1` on their final stage it was the slowest of the few riders stored, not the lanterne rouge. `scrape_gc_last_finisher.py` reads the figure off PCS's own GC page: **13 Giro years + Vuelta 1995 researched, 12 overrides written**, and the errors were large — Giro 1928 charted the lanterne rouge 3h51m behind when the real gap is **22h43m** (+67,925s), 1930 +45,830s, 1931 +54,508s. **0 editions now chart a figure taken from the handful of riders stored.** Giro 1964 and 1967 were already right and got no override, because a no-op override is stripped by `audit_summary_overrides.py` anyway. Two remain open and neither charts anything today: **Giro 1912** has no individual GC at all (the edition was decided on team points) and **Giro 1946** fails the gate below. **Vuelta 1978 was left alone** — it already carries a researched override of 321,329s and PCS says 317,094s, 4,235s apart, and overwriting a researched value with a second source is a decision rather than a fix.
  **The parse is gated on a number sourced independently.** Every page's winner time must equal `{race}_gc_winner_times.json` to the second or the year is refused, which is what caught Giro 1946: PCS's page gives a winner time of 235,940s, and 3,050 km in 65.5 hours is 46.6 km/h, so something on that page is not what it looks like. A gate that only checked "did it parse" would have written it.
  **Two traps re-used rather than re-discovered**, both already solved in this repo. The first `<table class="results">` on a PCS GC page is **not** the GC — it is whichever tab renders first, and reading it returns a confident, wrong 10-row table; `tab_blocks()` keys each block by the page's own nav. And PCS prints the value twice in one cell (`111:10:48 111:10:48`, `4:11:234:11:23`).
  **"Slowest" is the largest TIME, not the last rank.** Giro 1961 separates them: rank 92 is 2:36:40 down while rank 91 is 3:22:20 down, because PCS has adjusted the last man's time — its `*` marker, the same one the GC legend shows. Taking the last row would have understated 1961 by 46 minutes.
  `validate_db` now counts the research record as coverage, so it reports **2 of 17** rather than naming editions whose figure is already known.
  **Giro 1946: PCS is wrong, and PCS proves it.** The gate refused this year because the page's winner time is 65:32:20 (235,940s), which over our stored 3,050 km is 46.5 km/h. Checking it against PCS's own pages settles it: its **route page lists 20 stages summing to 3,189 km**, and its **GC page states "Avg. speed winner: 33.369 km/h"**. Those two numbers imply a winning time of **95:34:04** — PCS's published GC time is **30h01m short** of what the rest of PCS says. Run the other way, 3,189 km in 65:32:20 is **48.7 km/h**, against the 34.3 km/h of the one 1946 stage time we hold (itself PCS-sourced). The riders on that page are right — Bartali winning, Coppi second at **0:47**, and Luigi Malabrocca last, the original *maglia nera* — so it is the time field alone that is broken, not the classification.
  **Giro 1946 is now filled, from Wikipedia (2026-09-19).** Wikipedia's 1946 Giro infobox and GC table both give Bartali **95h 32' 20" = 343,940s**, and the decisive detail is that the minutes and seconds are IDENTICAL to PCS's 65:32:20 — **PCS mistyped one digit, 9 -> 6**. Three things agree: Wikipedia's figure; PCS's own `Avg. speed winner: 33.369 km/h` over its own 3,189 km route, which implies 95:34:04, **within 104 seconds**; and PCS's own GAPS, which are correct (Coppi +47", Ortelli +15'28" match Wikipedia exactly). Only the winner's absolute time was broken, which is why the gaps beside it could be trusted. So `giro_gc_winner_times.json` gains `1946: 343940` and the override becomes 343,940 + Malabrocca's 14,984 = **358,924s**. At our stored 3,050 km that is 31.9 km/h for the winner and 30.6 km/h for the lanterne rouge, inside the 26.9-36.7 band of the 13 years filled in `4f82d99`. Ten riders in `gc_by_stage_1946.json` gained a `totalTimeSeconds` as a side effect, since one can now be computed at all.
  **That file's `_README` had recorded 1946 as DELIBERATELY MISSING** with exactly this open question — "not clear whether the time or the distance is at fault" — and it now records the answer instead. **The three distances still disagree** (ours 3,050, PCS's route 3,189, Wikipedia's 3,039.5) and were left alone: that is a separate question and none of it changes the time. **Giro 1912 is not a gap — it is a race that had no individual GC**, and it turned out we were asserting one anyway (see below).

### The 1912 Giro had no individual classification, and we had invented one (2026-09-19)

Wikipedia is explicit: *"In 1912, Giro was contested solely by teams, with no individual classification."* The organisers moved the general classification onto teams and allowed only four-rider squads; the published result is a **team**, Atala-Dunlop in **100h 02' 57"**, ahead of Peugeot and Gerbi.

**We stored ten individual GC ranks for it.** Stage 1 held `gc_rank` 1-10 with `gc_gap_seconds` 0, matching `stage_rank` exactly, and no other stage had any — the signature of `ingest_race`'s stage-1 fallback, which gives a rider their STAGE placing when nothing else supplies a GC. On every other edition that approximates something real. On 1912 it invents a classification the race never held, and the app carried it: `gcRank: 1` on Micheletto's stage 1 in `gc_by_stage_1912.json`.

**Fixed in the ingest, not just the database**, because the fallback would have recreated it on the next re-ingest. `race_common.NO_INDIVIDUAL_GC` names the edition, `ingest_race` checks it *before* the other conditions, `validate_db` exempts it from the partial-classification check (nothing can ever fill it, so it is not a worklist item), and the 10 rows were nulled. The 80 stage-result rows stay: the stage results are real, only the classification was not.

`test_an_edition_with_no_individual_gc_gets_no_approximation` covers it, and was checked the other way — with the guard disabled it fails, so it is testing the guard rather than the weather.

With 1912 exempted and 1946 filled, the check now reports a **note** instead of a warning: all 16 editions with a partial final classification carry a researched figure.
...
- **1905–1912 gc_gap_seconds** (re-measured 2026-09-19, and the old note here was wrong). It said "PCS stored intra-stage gaps, not cumulative race time gaps". **They are cumulative race-long gaps.** Across all eight points-system years there are 4,412 mid-race rows with a gap and **0 inversions** — no rider ever carries a smaller gap than someone ranked ahead of him — which an intra-stage gap could not manage, and the gaps are plainly independent of that day's stage time (Tour 1910 st8 puts three riders on an identical 23,040s stage time at gaps of 3,000, 3,960 and 5,641). So the mid-race column IS usable.
  **The anomaly is the final stage alone: 271 of its 272 rows are 0**, one per year across 1905–1912, because PCS's final classification for those years is the POINTS table and carries no times. The single exception is Tour 1909 rank 56, Lucien Léman, at 17,115s. Those zeros reach the app: `gc_by_stage_1910.json` shows all 41 riders in the final GC at `+0:00`, which reads as "everyone finished level with Lapize" when they finished hours apart. Same shape as the Giro 1912 fabrication — a time-valued field asserting something about a classification that was not decided on time.
  **APPLIED 2026-09-19: 316 placeholder gaps nulled.** More than the 271 counted on final stages, because the rule is not "final stage" but **"every ranked rider on this stage is 0"** — which is what separates a placeholder column from an ordinary stage, where exactly one rider (the leader) legitimately sits at 0. Six 1905 stages and a handful elsewhere are all-zero too. What is left is 12-16 zeros a year: one per stage, the leader's own.
  **Unlike the Giro 1912 case this is NOT our fabrication — PCS publishes the `+0:00`.** `tour_scrapes/1910/stage_15.json` has all 41 ranked rows at `+0:00`, and `gc_standings.json` repeats it. That is why the fix had to go in the ingest: a re-ingest would otherwise restore every one of them. `race_common.POINTS_CLASSIFICATION` names the eight editions and `ingest_race` suppresses a gap only where the whole stage is zero.
  **Only a zero is suppressed, never a value.** Tour 1909's final classification is all `+0:00` in the stage file, yet `gc_standings` gives Lucien Léman a real **+4:45:15 (17,115s)** — the single non-zero in 272 final-stage rows. A blanket "null the gaps on this stage" would have deleted it; `test_a_real_gap_from_standings_survives_the_suppression` pins that down.
  **Verified by round trip**, which is the property that actually matters: re-ingesting Tour 1910 into a COPY of the database reproduced the fixed state on **0 differing rows** — 14 zeros (the leaders') and 111 nulls. The re-export moved **316 `gcGapSeconds` and nothing else**.
