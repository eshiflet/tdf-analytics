...
- **342 stages hold a GC ladder that runs backwards**, reported by `validate_db.check_gc_gap_monotonicity()` and triaged by **`audit_gc_ladders.py`** (offline, read-only). The GC *is* the ranking of aggregate time, so a later rank stored closer to the leader is arithmetically impossible — Tour 1919 st3 has rank 23 at 30,667s and rank 24 at 1s. Found 2026-09-19 when the 241 tie-groups turned out to live in only 49 stages while 293 more were corrupt without anything colliding.
  **The offline repair was tried and rejected, and the reason is the useful part.** *Rank is not recoverable from gap*: riders on equal aggregate time take DIFFERENT ranks, split by a tiebreak (sum of placings) we do not store — deriving rank from gap alone moves **71,806 rows, 18% of the archive**. Gap is not recoverable from rank for the same reason reversed. So the ladder proves a row is wrong without saying which value is right, and an offline "fix" would be fabrication.
  What the ladder CAN do is name the row and bound it, which is the triage: **144 ONE ROW** (removing exactly one of the step's two rows makes the ladder monotone, so that row is the suspect and its true gap is bounded by its neighbours — `rider/lucien-didier`, Tour 1978 st8, stores 4s where the ladder needs 289-304s), **75 PAIR** (either row would explain it; the evidence does not choose), **123 LADDER** (several steps, or neither removal is enough — re-fetch the classification whole). **24 stages hold TWO interleaved classifications**, one with `gc_rank` copied from `stage_rank`: Tour 1987 st1 stores ranks 13-18 twice, once at a flat 23s and once at the real prologue gaps. That is a ladder to REMOVE, not a value to correct.
  ...
  **RESOLVED 2026-09-19, and the re-scrape was never needed: most of these are PCS's own and our values are right.** PCS prints an asterisk on a rider whose PLACE the jury changed while his TIME stood — "Tim Merlier relegated from 2nd to 89th" on Giro 2024 st11, whose page carries a Penalties & Fines tab saying so. His stored time is then smaller than that of the riders now ranked above him, which is exactly this shape. The separation is total: across every stored GC page, **717 asterisked rows are out of order and 227,820 unasterisked rows are not** — 100% against 0.00%. Our scraper keeps the marker in the raw cell (`*0:040:04`, the visible text and PCS's hidden sort span concatenated) and **the ingest drops it**, so the contradiction downstream is the LOST MARKER, not a wrong number.
  `audit_gc_ladders.py` now reads the stored page and says so per stage: **230 EXPLAINED, 58 UNEXPLAINED, 54 with no page on disk**. Verified the other way too — on **126 of 127** ONE ROW stages with a local page, every stored rank and gap matches the source exactly. **The repair is to preserve the marker at ingest** (a jury-adjusted flag on `stage_results`, exempted by the validator the way `disqualified` already is), not to touch a single value. Only the 58 UNEXPLAINED are a data question.
...
- **The 1992 Giro's stored GC is one stage stale**, found 2026-09-19 while cross-checking GC ladders against the stored pages. Stages 2-20 match the PREVIOUS stage's classification at **100%** (180/180, 179/179, ...) and their own at 0-20%; stages 21-22 are correct. 2,063 rows — every rider's GC position and gap for most of that race is a day behind. This is the single biggest source disagreement in the archive: the other five editions that disagree at all hold **22 rows between them**. Not applied; the fix is offline (the right pages are already in `giro_scrapes/1992/gc_pages/`) but it rewrites a whole edition's GC and wants a change table first.
- **921 NULL `gc_gap_seconds` can be filled from pages already on disk**, across 22 editions (Tour 1923 alone holds 300, Tour 2006 164, Tour 1903 71). A NULL-fill carries no revert risk — nothing is overwritten — but it has not been applied. Measured against 3,881 stages that have a local GC page.
- **Tour 1926 st17 has two riders' rank AND gap swapped**: `rider/leon-parmentier2` is 6th at 6,975s and `rider/albert-dejonghe` 7th at 7,760s on the page, and the database has them the other way round. One pair, one stage — but read [[project_name_swap_repair]] before touching it, since a re-scrape UNDOES a repair.
...
- **Two struck riders are not flagged**, the only gaps in the disqualification sweep, found 2026-09-19: `rider/tadej-valjavec`, struck at 39 in a Vuelta 2009 classification, holds 21 stage rows and none is flagged; `rider/ramon-carretero`, struck at rank 18 on Giro 2015 stage 1, holds 2 rows and neither is flagged. PCS strikes both and this database flags neither, which is the whole claim — WHY PCS struck them is not recorded here and should be read before applying anything. `audit_disqualifications.py --race giro --years 2015 --apply` is the tool; nothing has been applied. Both were invisible to the sweep's own `--duplicated-rank1` entry point because no rider was promoted into either vacated place, which is the shape that leaves no trace in our data at all.
